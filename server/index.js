...
const {client_id: clientId, client_secret: clientSecret, port, rate_limit: rateLimit, NODE_ENV} = process.env
...
.get('/auth', async (ctx, next) => {
  ...
  ctx.redirect(`${endpoint}/oauth/authorize?client_id=${clientId}&scope=read_qiita&state=${token}`)
  await next()
})
.get('/auth/callback', async (ctx, next) => {
  ...
  await rp({
    ...
    body: {clientId, clientSecret, code},
    json: true
  })
    .then(auth => {
      ctx.assert(auth.client_id === clientId, 500, null, {detail: 'Internal OAuth Request Failed'})
      return Promise.all([
        auth.token,
        rp({
...
.use(ratelimit({duration: 60000, rate: +rateLimit || 30, id: ctx => `${ctx.method}${ctx.user}${ctx.ip}`, throw: true}))
.use(router.routes())
